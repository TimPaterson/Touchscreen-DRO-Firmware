...
public:
	bool Init()
	{
		...
		// GT9271 doesn't like the pull-up when it's back to an input
		SetPinConfigB(PORT_PINCFG_INEN | PORT_PINCFG_PMUXEN, TouchIrq_BIT);
		ClearTouchIrq();
		SetTouchIrqOut();
		...
		SetCtpReset();
		Timer::Delay_ms(50 + 5);
		SetTouchIrqIn();	// Re-enable TouchIrq_PIN as input (muxed to EXINT 16)
		...
		// Turn TouchIrq_PIN pullup back on
